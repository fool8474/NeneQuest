...
class mainFrame extends JFrame implements Runnable, KeyListener {
    ...
    private Image point;
    private Image hero[];
    private Image heroShadow;
    private Image heroDamaged;
    private Image live;
    private Image monster[];
    private Image monsterLDeath[];
    private Image monsterRDeath[];
    private Image bossDeath[];
    private Image boss[];
    ...
    private Image swordAttacked;
    private Image swordAttackAttacked;
    private Image upper;
    private Image bottom;
    private Image Ending;
    private Image backgroundBoss;
    private Image backgroundSky;
    private Image background;
    private Image backgroundOrnaments[];
    ...
    private Boss bs;
    private Bullet bu;
    private int scene; // 0 : 타이틀, 1 : 플레이, 2 : 엔딩
    private int menu;
    private int level;
    ...
    private int charY;
    private int lives;
    private int charStatus; // 0 : 평상시, 1 : 공격
    private int score;
    private int nx;

    private double playerJumpMathVar;
    private double attackDamage;

    private int bossRandWalkY;
    private int bossRandWalkX;
    private int bossAttackNum;
    private int bossAttackCool;
    private int bossAttackStayTime;
    private int bossSpecialAttackNum;
    private int bossSpecialAttackCool;
    private double calCntBulletSize;
    ...
    private boolean afterBossDie;
    private boolean success;
    private boolean successWav;
    private boolean gameOverWav;
    private boolean titleWav;
    private boolean fightWav;
    private boolean endWav;
    private boolean changeScene;
    private boolean gameOn;
    private boolean onJumped;
    private boolean onPause;
    private boolean whileAttacked;
    private boolean tutorial;

    private int cnt;
    private int cntBossBx;
    private int cntBx;
    private int cntItem;
    private int cntJump;
    private int cntAttack;
    private int cntAttackCool;
    private int cntGameOver;
    private int cntSuccess;
    private int cntStartLevel;
    private int cntEnd;
    private int cntSceneChange;
    private int cntSceneWait;
    private int cntTutorial;
    private int attackCoolTime;
    private int walkNum;

    private Clip[] clip; // 0배경
    private Clip[] clips; // 0킬 1먹기 2보스컷

    private boolean onSuccessionDamaged;
    private boolean onDamaged;
    private int damageCoolTime;
    private int damageSuccessionCoolTime;


    private boolean attackCool;

    private int createMonTiming;
    private int createItemTiming;

    private int orNum;

    private boolean keyUp, keyDown, keyRight, keyLeft, keyK, keyJ;
    ...
    private void init() {
        hero = new Image[2];
        monster = new Image[4];
        monsterLDeath = new Image[8];
        monsterRDeath = new Image[8];
        bossDeath = new Image[8];
        boss = new Image[2];
        backgroundOrnaments = new Image[4];
    ...
    private void damageProcess(int type, int damage) {
        switch (type) {
            case 1://한번 하고 사라지는타입
    ...
    private void gameProcess() {
        ...
        for (Clip curClip : clips) {
            if (curClip.getFrameLength() == curClip.getFramePosition())
                ...
                if (bs.live) {
                    ...
                    switch (bs.bossState) {
                        ...
                        case 3:
                            ...
                            } else if (bossAttackStayTime % 2 == 0)
                                bs.y += 7;
        ...
        switch (scene) {
            ...
            case 1:
                if (!bossActive && !afterBossDie) {
                    ...
                    if (keyUp) {
                        if (charY >= 20 && (charX <= cntBossBx + 1450 || charX >= cntBossBx + 1750 || charY >= 160)) {
                            charY -= 5;
                        }
                    }
                    if (keyDown) {
                        if (charY <= frameY - 350 && (charX <= cntBossBx + 1450 || charX >= cntBossBx + 1750 || charY <= 320)) {
                            charY += 5;
                        }
                    }
                    if (keyRight) {
                        if (charX <= frameX - 250 && (charX <= cntBossBx + 1400 || (charY <= 350 && charY >= 150) || charX > cntBossBx + 1600)) {
                            charX += 5;
                        }
                    }
                    if (keyLeft) {
                        if (charX >= 25 && (charX >= cntBossBx + 1850 || (charY <= 350 && charY >= 150) || charX <= cntBossBx + 1600)) {
                            charX -= 5;
                        }
                    }
                }
    ...
    private boolean CrashWithXY(int img1X, int img1Y, int img2X, int img2Y, int img1Width, int img1Height, int img2Width, int img2Height) {
        boolean result;

        result = (Math.abs((img1X + img1Width / 2) - (img2X + img2Width / 2)) < ((img2Width / 2) + (img1Width / 2))) &&
                (Math.abs((img1Y + img1Height / 2) - (img2Y + img2Height / 2)) < ((img2Height / 2) + (img2Height / 2)));

        return result;
    }

    private void addSound(String file, boolean Loop, int n) {
        try {
    ...
    private void backSound(String file, boolean Loop, int i) {
        try {
